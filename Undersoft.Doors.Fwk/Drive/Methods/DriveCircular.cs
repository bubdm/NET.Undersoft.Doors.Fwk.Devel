...
namespace System.Doors.Drive
{
    ...
    public unsafe class DriveCircular : Drive
    {
        #region Structures
        ...
        public DriveCircular(string file, string name, int nodeCount, long nodeBufferSize) : 
                             this(file, name, nodeCount, nodeBufferSize, true)
        {
            Open();
            CheckArchive();
        }
        public DriveCircular(string file, string name) : 
                             this(file, name, 0, 0, false)
        {
            Open();
        }

        private DriveCircular(string file, string name, int nodeCount, long nodeBufferSize, bool ownsSharedMemory) : 
        ...
        private void InitialiseNodeHeader()
        {
            ...
            object head = header;
            base.Write(ref head, NodeHeaderOffset);
        }
        private void InitialiseLinkedListNodes()
        {
            ...
            base.WriteArray(nodes.SelectMany(o => new object[] { o }).ToArray(), 0, nodes.Length, NodeOffset, typeof(Node));
        }
        ...
        public virtual int Write(ref object source, int startIndex = 0, Type t = null, int timeout = 1000)
        {
        ...
        public virtual int Write(object[] source, int startIndex = 0, Type t = null, int timeout = 1000)
        {
            ...
            // Write the data using the RawDrive class (much faster than the MemoryMappedViewAccessor WriteArray<T> method)
            long count = Math.Min(source.Length - startIndex, NodeBufferSize / Marshal.SizeOf(source.GetType()));
            base.WriteArray(source, startIndex, (int)count, node->Offset);
        ...
        public virtual int Read(out object destination, int startIndex = 0, Type t = null, int timeout = 1000)
        {
        ...
        public virtual int Read(ref object[] destination, int startIndex = 0, Type t = null, int timeout = 1000)
        {
            Node* node = GetNodeForReading(timeout);
            if (node == null) return 0;

            // Copy the data using the RawDrive class (much faster than the MemoryMappedViewAccessor ReadArray<T> method)
            int count = (int)Math.Min(destination.Length - startIndex, node->AmountWritten / Marshal.SizeOf(t));
            base.ReadArray(ref destination, startIndex, count, node->Offset);
