...
namespace System.Doors.Drive.Utilities
{
    [SecurityPermission(SecurityAction.LinkDemand)]
    [SecurityPermission(SecurityAction.InheritanceDemand)]
    public struct DriveSlicer : IList, ICollection
    {
        private readonly IList _list;
        private readonly int _offset;
        private readonly int _count;

        public DriveSlicer(IList list)
        {
            if (list == null)
        ...
        public DriveSlicer(IList list, int offset, int count)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (offset < 0)
        ...
        {
            if (obj is DriveSlicer)
                return Equals((DriveSlicer)obj);
            else
                return false;
        }

        public bool Equals(DriveSlicer obj)
        {
            return obj._list == _list && obj._offset == _offset && obj._count == _count;
        }

        public static bool operator ==(DriveSlicer a, DriveSlicer b)
        {
            return a.Equals(b);
        }
        ...
        private sealed class ArraySliceEnumerator : IEnumerator
        {
            ...
            internal ArraySliceEnumerator(DriveSlicer arraySlice)
            {
