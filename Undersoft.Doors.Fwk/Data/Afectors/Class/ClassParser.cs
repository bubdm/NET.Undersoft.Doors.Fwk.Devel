...
namespace System.Doors.Data.Afectors.Class
{
  
    /// <summary>
    /// Exception raised when <see cref="ClassParser" /> encounters an invalid token.
    /// </summary>
    public class InvalidClassParserException : Exception
    {
        public InvalidClassParserException(string message)
            : base(message)
        {
    ...
    public class ClassArray : DynamicObject, IEnumerable, IClassParser
    {
        private readonly List<IClassParser> _collection;

        public ClassArray(ICollection<object> collection)
        {
            _collection = new List<IClassParser>(collection.Count);
            foreach (var instance in collection.Cast<IDictionary<string, object>>())
            {
                _collection.Add(new ClassObject(instance));
            }
        }
    ...
    public class ClassObject : DynamicObject, IClassParser
    {
        private readonly IDictionary<string, object> _hash = new Dictionary<string, object>();

        public ClassObject(IDictionary<string, object> hash)
        {
            _hash = hash;
        }
        ...
        private bool YieldMember(string name, out object result)
        {
            if (_hash.ContainsKey(name))
            {
                ...
                {
                    result = new ClassObject((IDictionary<string, object>)result);
                    return true;
                }
    ...
    {
        private const NumberStyles ClassNumbers = NumberStyles.Float;
        ...
        public static string ToSphere(IDictionary<string, object> bag)
        {
        ...
        internal static void SerializeItem(StringBuilder sb, object item)
        {
            ...
            if (item is IEnumerable && item.GetType().FullName != "System.String")
            {
            ...
            {
                double numb;
                var inp = item != null ? item.ToString() : "";
#if NETCF
            if (input.TryParse(out numb))
            {
                sb.Append(numb);
            }
#else
                if (double.TryParse(inp, ClassNumbers, nfi, out numb))
                {
                    sb.Append(numb);
                    return;
                }
#endif
                if (item.ToString().Last<char>() != '"')
                {
                    sb.Append("\"" + item + "\"");
                }
                else
            ...
            if (item is Decimal)
            {
                ...
                sb.Append(Convert.ToDouble(item).ToString(nfi));
                return;
            }
...
#else
            if (double.TryParse(input, ClassNumbers, nfi, out number))
            {
                sb.Append(number);
                return;
            }
...
#else
            if (!double.TryParse(buffer, ClassNumbers, CultureInfo.InvariantCulture, out result))
            {
                throw new InvalidJsonException(
                    string.Format("Value '{0}' was not a valid JSON number", buffer)
                    );
